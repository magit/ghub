...
#+SUBTITLE: for version 3.3.0 (v3.3.0-10-g961cd30+1)
...
Ghub is an Elisp library that is used by packages to access the
GraphQL and/or REST APIs of various forges such as Github, Gitlab and
Gitea, hosted on https://github.com, https://gitlab.com and elsewhere.

Most users will get in contact with Ghub and this manual when using
the Forge package.

#+TEXINFO: @noindent
This manual is for Ghub version 3.3.0 (v3.3.0-10-g961cd30+1).
...
- Unfortunately only Github supports the creation of tokens by using
  ...
  create the token manually as described in [[*Manually Creating and
...
- If you want to access a Github Enterprise instance, then you have
  ...
  For some Github Enterprise instances ~/api~ has to be used instead of
...
Note that in the above example ~github.example.com/api/v3.user~ is a Git
variable whose name has the form ~github.HOST[/PATH].user~.  If the API
were available at ~github.example.com/api/v3~, then the variable would
be ~github.github.example.com/api/v3.user~.
