...
#+SUBTITLE: for version 1.2.0 (v1.2.0-60-g97edf6d+1)
...
#+BEGIN_QUOTE
This manual is for Ghub version 1.2.0 (v1.2.0-60-g97edf6d+1).
...
then the setup wizard will first ask you this:

#+BEGIN_EXAMPLE
  Username [for https://USERNAME@api.github.com]: USERNAME
#+END_EXAMPLE

Unfortunately Ghub has no control over this, so it cannot present a
more meaningful prompt.  Confirm the default choice by pressing ~RET~.
If the shown USERNAME is not your Github.com (or Github Enterprise
instance) username, then you have done something wrong during the step
describe in the previous section (which see).

Then you will also be asked for a password.  Provide the password that
you use when accessing Github.com or the Github Enterprise instance in
a browser.  Finally you are asked something like:
...
- When creating a token it is necessary to provide a token
  ...
  be unique and in rare cases you might see this warning:
